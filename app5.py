...
if hours1 in [1,2,3,4]:
    st.markdown(f"<h1 style='text-align:center;'>{current_time}</h1>", unsafe_allow_html=True)
    sleep ="AI 「今日は遅いのでもう寝ましょう！！  \nまた明日会いましょう！！おやすみなさい」"
    st.markdown(f"<font color='green'>{sleep}</font>", unsafe_allow_html=True)
else:
    st.markdown(f"<h1 style='text-align:center;'>{current_time}</h1>", unsafe_allow_html=True)
